    ...
    >
      ...
      {loading ? (
        ...
        <div className="space-y-3 flex-1 flex flex-col items-center justify-center text-center">
          ...
          <h4 className="text-slate-200 font-bold text-sm">No Pending Tasks</h4>
        ...
        <div className="space-y-3 overflow-y-auto max-h-[200px] pr-1 custom-scrollbar">
          {tasks.map((task, idx) => (
            <div key={idx} className="flex items-center gap-3 p-3 rounded-xl bg-slate-800/40 border border-slate-700/50 hover:bg-slate-800/60 transition-colors group cursor-pointer" onClick={() => navigate(`/assessment/${task.id}`)}>
              <div className={`p-2 rounded-lg ${task.color}`}>
                <task.icon className="w-4 h-4" />
              </div>
              <div className="flex-1 min-w-0">
                <h4 className="text-sm font-semibold text-slate-200 truncate group-hover:text-white transition-colors">{task.title}</h4>
