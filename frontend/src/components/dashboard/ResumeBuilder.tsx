...
const ResumeBuilder: React.FC = () => {
  ...
  useEffect(() => {
    const loadResume = async () => {
      try {
        ...
        if (token) {
          try {
            const data = await resumeService.getMe();
            if (data && data.resume) {
              setResumeData(prev => ({
                ...initialResumeState,
                ...data.resume,
                header: { ...initialResumeState.header, ...(data.resume?.header || {}) },
                experience: data.resume?.experience || [],
                education: data.resume?.education || [],
                projects: data.resume?.projects || [],
                skills: { ...initialResumeState.skills, ...(data.resume?.skills || {}) }
              }));
              setLastSaved(new Date()); // Assume fresh from backend
            }
          } catch (apiError) {
            console.error("Failed to load resume from backend", apiError);
            // Don't fallback to local storage if backend fails, strictly use backend
          }
        }
      } catch (e) {
  ...
  // Auto-save to backend when data changes (debounced by 2 seconds)
  useEffect(() => {
    // Only auto-save if it's different from initial state (meaning user has made edits)
    if (JSON.stringify(resumeData) === JSON.stringify(initialResumeState)) return;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem('authToken');
      if (token) {
        await resumeService.saveMe(resumeData);
        setLastSaved(new Date());
      }
    } catch (error) {
      console.error("Failed to save resume", error);
      alert("Failed to save resume to backend.");
    } finally {
  ...
  const toBulletItems = (text: string) => {
    ...
    const lines = cleaned
      ...
      .map((line) => line.trim())
      .filter(Boolean)
      .flatMap((line) => line.split(/(?<=\.)\s+(?=[A-Z0-9])/g).map((part) => part.trim()).filter(Boolean));
  ...
  const handleExport = () => {
    const safe = {
      ...
      email: escapeHtml(resumeData.header.email || ''),
      phone: escapeHtml(resumeData.header.phone || ''),
      location: escapeHtml(resumeData.header.location || ''),
      website: escapeHtml(resumeData.header.website || ''),
      summary: escapeHtml(resumeData.header.summary || '')
    };

    const contactParts = [safe.email, safe.phone, safe.location, safe.website].filter(Boolean);
    ...
    const experienceHtml = (resumeData.experience || [])
      ...
      .map((exp) => {
        ...
        const start = escapeHtml(exp.startDate || '');
        const end = exp.current ? 'Present' : escapeHtml(exp.endDate || '');
        const dateRange = [start, end].filter(Boolean).join(' - ');
        const bullets = toBulletItems(exp.description || '');
        return `
          <section class="entry">
            <div class="entry-head">
              <h3>${role}${role && company ? ', ' : ''}${company}</h3>
              <p>${dateRange}</p>
            </div>
            ${bullets}
          </section>
    ...
    const educationHtml = (resumeData.education || [])
      .filter((edu) => edu.school || edu.degree || edu.field || edu.description)
      .map((edu) => {
        const school = escapeHtml(edu.school || '');
        const degree = escapeHtml(edu.degree || '');
        const field = escapeHtml(edu.field || '');
        const start = escapeHtml(edu.startDate || '');
        const end = edu.current ? 'Present' : escapeHtml(edu.endDate || '');
        const dateRange = [start, end].filter(Boolean).join(' - ');
        const heading = [degree, field].filter(Boolean).join(' in ');
        const bullets = toBulletItems(edu.description || '');
        return `
          <section class="entry">
            <div class="entry-head">
              <h3>${heading}${heading && school ? ', ' : ''}${school}</h3>
              <p>${dateRange}</p>
            </div>
            ${bullets}
          </section>
    ...
    const projectHtml = (resumeData.projects || [])
      .filter((proj) => proj.name || proj.description || proj.technologies || proj.url)
      .map((proj) => {
        const name = escapeHtml(proj.name || '');
        const tech = escapeHtml(proj.technologies || '');
        const url = escapeHtml(proj.url || '');
        const bullets = toBulletItems(proj.description || '');
        return `
          <section class="entry">
            <div class="entry-head">
              <h3>${name}</h3>
            ...
            ${tech ? `<p class="meta"><strong>Technologies:</strong> ${tech}</p>` : ''}
            ${bullets}
          </section>
        `;
      })
    ...
    const skillsHtml = [resumeData.skills.technical, resumeData.skills.soft]
    ...
    const html = `
      ...
      <body>
        <main class="page">
          ...
          ${safe.summary ? `<section><h2>Professional Summary</h2><p>${safe.summary}</p></section>` : ''}
          ...
          ${skillsHtml ? `<section><h2>Skills</h2><p>${skillsHtml}</p></section>` : ''}
        </main>
      </body>
      </html>
    `;
