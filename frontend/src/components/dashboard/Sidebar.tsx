...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  React.useEffect(() => {
    // Calculate Level & XP from completed assessments
    const { level, xp } = calculateUserLevel();
    
    setLevel(level);
    setXp(xp);
    setMaxXp(1000); // Fixed for now, could scale
  }, []);
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-xs font-medium text-slate-400">
                <span className="flex items-center gap-1"><Zap className="w-3 h-3 text-amber-400" /> Lvl {level}</span>
